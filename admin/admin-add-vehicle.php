  ...
  if(isset($_POST['add_veh']))
    {
            ...
            $v_reg_no =  htmlspecialchars ($_POST['v_reg_no']);
            $v_category= htmlspecialchars($_POST['v_category']);
            $v_pass_no= htmlspecialchars($_POST['v_pass_no']);
            $v_status= htmlspecialchars($_POST['v_status']);
            $v_driver= htmlspecialchars ($_POST['v_driver']);
            $v_dpic=($_FILES["v_dpic"]["name"]);

            $allowed_extensions = array('png');
            $file_extension = pathinfo($_FILES['v_dpic']['name'], PATHINFO_EXTENSION);
        
            if (!in_array($file_extension, $allowed_extensions)) {
                $err = "Only PNG files are allowed!";
            } else {
        ...
        <div class="card-body">
          ...
          <form method ="POST" enctype="multipart/form-data"> 
            <div class="form-group">
                ...
                <input type="text" pattern="[a-zA-Z0-9 ]+" title="Only alphanumeric characters are allowed" required class="form-control" id="exampleInputEmail1" name="v_name">
            </div>
            <div class="form-group">
                <label for="exampleInputEmail1">Vehicle Registration Number</label>
                <input type="text" pattern="[a-zA-Z0-9]+" title="Only alphanumeric characters are allowed" class="form-control" id="exampleInputEmail1" name="v_reg_no">
            </div>
            <div class="form-group">
                <label for="exampleInputEmail1">Vehicle  Number Of Seats</label>
                <input type="number" min="1" title="only numbers are allowed"class="form-control" id="exampleInputEmail1" name="v_pass_no">
            </div>
            <div class="form-group">
            ...
            <label for="exampleInputEmail1">Vehicle Picture (PNG only)</label>
